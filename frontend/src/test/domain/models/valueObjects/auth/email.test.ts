...
describe('Email', () => {
  ...
  describe('異常系', () => {
    ...
    it('@を含まないEメールアドレスは生成できない', () => {
      expect(() => new Email('testexample.com')).toThrow('Eメールアドレスの入力形式が不正です');
    });

    it('.を含まないEメールアドレスは生成できない', () => {
      expect(() => new Email('test@examplecom')).toThrow('Eメールアドレスの入力形式が不正です');
    });

    it('空白を含むEメールアドレスは生成できない', () => {
      expect(() => new Email('test @example.com')).toThrow('Eメールアドレスの入力形式が不正です');
    });

    it('ローカル部がないEメールアドレスは生成できない', () => {
      expect(() => new Email('@example.com')).toThrow('Eメールアドレスの入力形式が不正です');
    });
    ...
    it('41文字以上のEメールアドレスは生成できない', () => {
      ...
      expect(() => new Email(value)).toThrow('Eメールアドレスは40字以内で入力してください');
    });
  });
});
